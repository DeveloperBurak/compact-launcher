...
$(document).ready(() => {
  ...
  body.on('click', htmlClassToSelector(classDropdownButton), e => {
    ...
    const dropdownLists = button.siblings(htmlClassToSelector(classDropdownList));
    // TODO improve there
    if (dropdownLists.is(':hidden')) {
      if (!dropdownLists.hasClass('inner')) {
        const otherDropdowns = button.closest('#program-container').children('li').children(htmlClassToSelector(classDropdownList));
  ...
  body.on('mouseenter', htmlClassToSelector(classProgramCover), e => {
...
const generateList = (list, inner = false) => {
  let programListCover = $('<li key="' + list.name + '">' +
    '<button class="' + classDropdownButton + '">' + list.name + '</button>' +
    '</li>');
...
const renderButton = (value) => {
  if (value.hasOwnProperty('file')) {
    return '<li class="' + classProgramCover + '">' +
      '<button class="' + classProgramButton + ' col-sm-11" programName="' + value.name + '" image="' + value.image + '" execute="' + value.exePath + '">' +
      '<p class="float-left">' + value.name + '</p> ' +
      '</button>' +
      '<button  class="' + classDeleteProgramButton + ' col-sm-1  float-right" del="' + value.exePath + '">X</button>' +
      '</li>';
  }
};
